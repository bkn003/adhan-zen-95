...
interface NextPrayerCardProps {
  nextPrayer: Prayer;
  selectedLocation?: { mosque_name: string; district: string };
}

export const NextPrayerCard = ({ nextPrayer, selectedLocation }: NextPrayerCardProps) => {
  ...
  return (
    <div className="bg-gradient-to-br from-green-600 to-emerald-600 rounded-2xl p-6 shadow-lg">
      ...
      {selectedLocation && (
        <div className="mt-3 text-white/70 text-xs text-center">
          {selectedLocation.mosque_name}, {selectedLocation.district}
        </div>
      )}
    </div>
  );
